...
nValInterval = 1            # Run on validation set every nTestInterval epochs
snapshot = 1                # Store a model every snapshot epochs
backbone = 'resnet101'      # backbone of DeepLabV3Plus model
# useTensorboardX = False      # Use TensorboardX
...
# if useTensorboardX:
#     log_dir = os.path.join(save_directory, 'models', datetime.now().strftime('%b%d_%H-%M-%S') + '_' + socket.gethostname())
...
if resume_epoch != nEpochs:
    for epoch in range(resume_epoch, nEpochs):
        ...
        for ii, sample_batched in enumerate(trainloader):
            ...
            # Update weights
            # -----------------
            # writer.add_scalar('data/total_loss_iter', loss.item(), ii + num_img_train * epoch)
            optimizer.step()
            optimizer.zero_grad()
            aveGrad = 0
        ...
        if useVal and epoch % nValInterval == (nValInterval - 1):
            ...
            for ii, sample_batched in enumerate(valloader):
                ...
                if ii % num_img_val == num_img_val - 1:
                    ...
                    # writer.add_scalar('data/val_loss_epoch', running_loss_val, epoch)
