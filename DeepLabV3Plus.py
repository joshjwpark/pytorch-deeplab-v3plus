...
class ResNet(nn.Module):
	...
	def _load_pretrained_model(self):
		pretrain_dict = model_zoo.load_url(url='https://download.pytorch.org/models/resnet101-5d3b4d8f.pth',
										   model_dir='D:/Projects/Segmentation/models')
		model_dict = {}
...
def ResNet18(nInputChannels=3, os=16, pretrained=False):
	model = ResNet(nInputChannels, BasicBlock, [2, 2, 2, 2], os, pretrained=pretrained)
	return model

def ResNet34(nInputChannels=3, os=16, pretrained=False):
	model = ResNet(nInputChannels, BasicBlock, [3, 4, 6, 3], os, pretrained=pretrained)
	return model
...
class DeepLabV3_Plus(nn.Module):
	def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, freeze_bn=False, _print=True):
		...
		self.aspp1 = ASPP_module(inplanes=2048, planes=256, dilation=dilations[0])
		self.aspp2 = ASPP_module(inplanes=2048, planes=256, dilation=dilations[1])
		self.aspp3 = ASPP_module(inplanes=2048, planes=256, dilation=dilations[2])
		self.aspp4 = ASPP_module(inplanes=2048, planes=256, dilation=dilations[3])
